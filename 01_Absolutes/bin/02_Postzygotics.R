...
mechanicals %>%
  ...
  mutate(Cross = factor(Cross, levels=c("elegansXhybrid","graellsiiXhybrid",
                                        "hybridXhybrid",
                                        "hybridXelegans","hybridXgraellsii"))) -> mechanicals
rm(sample.size)
...
mechanical.tactile %>%
  ...
  mutate(Cross = factor(Cross, levels=c("elegansXhybrid","graellsiiXhybrid",
                                        "hybridXhybrid",
                                        "hybridXelegans","hybridXgraellsii"))) -> mechanical.tactile
rm(sample.size)
...
oviposition %>%
  ...
  mutate(Cross = factor(Cross, levels=c("elegansXhybrid","graellsiiXhybrid",
                                        "hybridXhybrid",
                                        "hybridXelegans","hybridXgraellsii"))) -> oviposition
rm(sample.size)
...
oviposition %>%
  ggplot() +
  facet_grid2(cols = vars(Cross), scales = "free_x", independent = "x") +
  geom_bar(aes(x=Ecology2, fill=ClutchesWEggs, linetype = Ecology), linewidth = l, color="black", position = "fill") +
  scale_fill_manual(values = c('#66c2a5','#fc8d62'), name = "Outcome") +
  scale_linetype_manual(values = c("solid","blank")) +
  scale_y_continuous(labels = percent) +
  theme_classic() +
...
fecundity %>%
  ...
  mutate(Cross = factor(Cross, levels=c("elegansXhybrid","graellsiiXhybrid",
                                        "hybridXhybrid",
                                        "hybridXelegans","hybridXgraellsii"))) -> fecundity
rm(sample.size)
...
fecundity %>% 
  ...
  geom_point(aes(x=Population, y=EggsPerClutch, shape=Ecology), size = 1) +
...
fertility %>%
  ...
  mutate(Cross = factor(Cross, levels=c("elegansXhybrid","graellsiiXhybrid",
                                        "hybridXhybrid",
                                        "hybridXelegans","hybridXgraellsii"))) -> fertility
rm(sample.size)
...
fertility %>% 
  ...
  geom_point(aes(x=Population, y=Fertility, shape=Ecology), size = 1) +
  scale_shape_manual(values = c(17, 16)) +
  scale_fill_manual(values = c("#7570b3","#e7298a")) +
  scale_y_continuous(labels = percent) +
  theme_classic() +
...
print(pptx, target = "../figures/04_PopsPostzygotics.pptx")
...
read_pptx(slide.size) %>%
  ...
  print(target = "../figures/05_MergedPostzygotics.pptx")

# Saving tidied data for statistics ####
save(tidied, file = "../results/02_Postzygotics.Rdata")
